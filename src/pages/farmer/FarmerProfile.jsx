...
export default function FarmerProfile({ user, onLogout, onNav }) {
  return (
    <>
      ...
      {user.digilockerData && user.digilockerData.land && (
        <div className="card" style={{ borderLeft: '4px solid var(--green)' }}>
          <div style={{ fontWeight: 700, fontSize: '.86rem', marginBottom: '10px' }}>🔵 DigiLocker Verified Details</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {[
              { label: 'Aadhaar', val: user.digilockerData.aadhaarMasked },
              { label: 'Land Area', val: user.digilockerData.land },
              { label: 'Village', val: user.digilockerData.village },
      ...
      {[
        { icon: '🌐', label: 'Language · ಭಾಷೆ', sub: null, right: <span style={{ background: 'var(--blue-bg)', color: 'var(--blue)', padding: '3px 9px', borderRadius: '6px', fontSize: '.72rem', fontWeight: 600 }}>EN | ಕನ್ನಡ</span> },
        { icon: '🏦', label: 'Bank Account', sub: 'SBI ××××4821 — Linked', right: '›' },
        { icon: '📋', label: 'Land Records · ಭೂ ದಾಖಲೆ', sub: `${user.digilockerData?.land || '2.5 acres'} · ${user.district}`, right: '›' },
        { icon: '📊', label: 'Transactions · ವ್ಯವಹಾರ', sub: '48 completed deals', right: '›' },
        { icon: '📈', label: 'Growth Dashboard', sub: 'View your analytics', right: '›', onClick: () => onNav('growth') },
